...
static bool IsCookWorkerProcess()
{
	return IsRunningCookCommandlet()
		&& FParse::Param(FCommandLine::Get(), TEXT("CookWorker"));
}
        ...
        const FWwiseSharedPlatformId& InPlatform, EWwiseExportDebugNameRule InExportDebugNameRule)
{
...

	{
		...
		const bool bIsCookWorker = IsCookWorkerProcess();
		...
		if (UNLIKELY(!CookingPlatform))
		{
			...
			if (!IWwiseProjectDatabaseModule::ShouldInitializeProjectDatabase() 
				...
				&& !bIsCookWorker)		// By The Book cooking needs to predefine the requested platforms. InEditor and OnTheFly should create them all the time.
			{
		...
		if (bIsCookWorker)
		{
			IWwiseProjectDatabaseModule* DBModule = IWwiseProjectDatabaseModule::GetModule();
			if (DBModule && !DBModule->GetProjectDatabase())
			{
				UE_LOG(LogWwiseResourceCooker, Display,
					TEXT("CookWorker: Instantiating Wwise Project Database"));
				DBModule->InstantiateProjectDatabase();
			}
		}
